...
\DontPrintSemicolon
\caption{Compute $\Phi(n)$ in $\widetilde{\Theta}(n^{2/3})$ time and $\widetilde{\Theta}(n^{1/2})$ space \cite{griff2023}.  See file \texttt{totientsumC.py} for an implementation.}
\KwData{$n \geq 1$}
\KwResult{$\Phi(n)$}
\Begin{
...
\DontPrintSemicolon
\caption{Compute $\Phi(n)$ in $\widetilde{\Theta}(n^{2/3})$ time and $\widetilde{\Theta}(n^{1/2})$ space \cite{griff2023}.  See file \texttt{totientsumD.py} for an implementation.}
\KwData{$n \geq 1$}
\KwResult{$\Phi(n)$}
\Begin{
    $a \gets \widetilde{\Theta}(n^{2/3})$; $b \gets \floor{n/a}$; $X \gets 0$; $Y \gets 0$; $Z \gets 0$; $m \gets 0$; $s \gets \isqrt{n}$ \label{4-2}
...
The next step is to move line A\ref{Algo4}.\ref{4-27} into phase 1.  The work this line does is essentially
...
Applying this edit to Algorithm \ref{Algo4} yields Algorithm \ref{Algo8}.  Lines \ref{4-2} and \ref{4-5} have had actions added to them, lines A\ref{Algo8}.\ref{8-18}--\ref{8-21} have been inserted, and line A\ref{Algo4}.\ref{4-27} has been replaced with line A\ref{Algo8}.\ref{8-31}.

\begin{algorithm}[H] \label{Algo8}
\DontPrintSemicolon
\caption{Compute $\Phi(n)$ in $\widetilde{\Theta}(n^{2/3})$ time and $\widetilde{\Theta}(n^{1/2})$ space \cite{griff2023}.  See file \texttt{totientsumE.py} for an implementation.}
\KwData{$n \geq 1$}
\KwResult{$\Phi(n)$}
\Begin{
    $a \gets \widetilde{\Theta}(n^{2/3})$; $b \gets \floor{n/a}$; $X \gets 0$; $Y \gets 0$; $Z \gets 0$; $m \gets 0$; $s \gets \isqrt{n}$; $d \gets b$
