...
\Begin{
    ...
    \For{$y=b$ \KwTo $1$}{ \label{13-42}
        ...
        \For{$x=2$ \KwTo $\isqrt{v}$}{ \label{13-45}
            \If{$\floor{v/x} > \isqrt{n}$ and $\floor{n/\floor{v/x}} \leq b$}{ \label{13-46}
                $m \gets m - M^\prime_{\floor{n/\floor{v/x}}}$ \label{13-47}
            }
        }
        $M^\prime_y \gets M^\prime_y + m$
...
\eqn{\sum_{x=2}^{\isqrt{n}} \min\left(\floordiv{n}{a}, \floordiv{n}{x^2}\right)}
\eqn{= \sum_{x=2}^{\isqrt{a}} \floordiv{n}{a} + \sum_{x=\isqrt{a}+1}^{\isqrt{n}} \floordiv{n}{x^2}}
\eqn{= \floordiv{n}{a} \cdot \left(\isqrt{a} - 1\right) + \Theta\left(\integral{\;\frac{n}{x^2}}{x}{\isqrt{a}+1}{\isqrt{n}}\right)}
\eqn{= \floordiv{n}{a} \cdot \left(\isqrt{a} - 1\right) + \Theta\left( - n \cdot \eval{x^{-1}}{x=\isqrt{a}+1}{\isqrt{n}} \right)}
\eqn{= \floordiv{n}{a} \cdot \left(\isqrt{a} - 1\right) + \Theta\left( - n \cdot \left(\frac{1}{\isqrt{n}} - \frac{1}{\isqrt{a}+1}\right)\right)}
\eqn{= \floordiv{n}{a} \cdot \left(\isqrt{a} - 1\right) + \Theta\left(\frac{n}{\isqrt{a}+1} - \frac{n}{\isqrt{n}}\right)}
...
Line \ref{14-27} does not get hit on each iteration of phase 1.  On an $x$-value where it gets hit, it consumes
...
The first batch ($x=b$) and last batch $(x=\isqrt{n})$ need special handling.  For the first batch, when $x=b$, the relevant section of the algorithm amounts to
...
By Lemma \ref{ouawrt4coi}, the first argument of the $\max$ function is negative.  (\todo When the analysis is done, ensure that $b \leq n^{1/3}$.  Otherwise, this lemma-application is invalid.)
\eqn{= b}

In the last batch, when $x=\isqrt{n}$, the relevant section of the algorithm amounts to
